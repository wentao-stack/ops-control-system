...
def seed_development_data(session: Session) -> None:
    ...
    # ── Dev fixture assets (idempotent) ─────────────────────────────────────
    dev_asset_ids = [
        "asset-dev-api-01", "asset-dev-worker-01",
        "asset-staging-web-01", "asset-staging-db-01",
        "asset-prod-edge-01", "asset-prod-observer-01",
    ]
    existing_dev = session.query(Asset.id).filter(Asset.id.in_(dev_asset_ids)).all()
    if len(existing_dev) < len(dev_asset_ids):
        _seed_dev_fixtures(session)

    # ── Always-seed real production servers (idempotent) ────────────────────
    _seed_real_servers(session)

    # ── Alerts, Changes, Runbooks (idempotent) ──────────────────────────────
    if session.query(Alert).first() is None:
        _seed_alerts(session)
    if session.query(Change).first() is None:
        _seed_changes(session)
    if session.query(Runbook).first() is None:
        _seed_runbooks(session)


def _seed_dev_fixtures(session: Session) -> None:
    now = _now()
    session.add_all([
...
def _seed_real_servers(session: Session) -> None:
    now = _now()
    real_servers = [
        ...
        ("host-b", "163.44.124.142", 2222, "wentao", "wentao-MS-7C91", "Ubuntu workstation — RTX 3090 AI dev machine (open-webui+ollama+llama)"),
    ]
...
def _seed_changes(session: Session) -> None:
    now = _now()
    session.add_all([
...
def _seed_runbooks(session: Session) -> None:
    now = _now()
    session.add_all([
